    ...
    try {
      ...
      if (signUpError) {
        ...
        switch(signUpError.status) {
          case 400:
            setErrors([{ field: 'form', message: 'Geçersiz kayıt bilgileri. Lütfen bilgilerinizi kontrol edin.' }])
            break
          case 422:
            setErrors([{ field: 'form', message: 'E-posta adresi veya şifre geçersiz format.' }])
            break
          case 429:
            setErrors([{ field: 'form', message: 'Çok fazla deneme yaptınız. Lütfen bir süre bekleyin.' }])
            break
          default:
            if (signUpError.message.includes('already registered')) {
              setErrors([{ field: 'email', message: 'Bu e-posta adresi zaten kayıtlı' }])
            } else {
              setErrors([{ field: 'form', message: 'Kayıt işlemi başarısız oldu: ' + signUpError.message }])
            }
        }
        return
      }
      ...
      if (!data?.user) {
        setErrors([{ field: 'form', message: 'Kayıt işlemi başarısız oldu. Lütfen daha sonra tekrar deneyin.' }])
        return
      }
      ...
      setErrors([{ 
        field: 'form', 
        message: error.message || 'Beklenmeyen bir hata oluştu. Lütfen daha sonra tekrar deneyin.'
      }])
    } finally {
      setLoading(false)
    }
        ...
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded relative" role="alert">
          {errors.map((error, index) => (
            <div key={index} className="mb-1">
              <strong className="font-bold">{error.field === 'form' ? 'Hata! ' : `${error.field}: `}</strong>
